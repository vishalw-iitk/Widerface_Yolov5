...
run-----> python /content/drive/MyDrive/Wider_Face/yolov5/Density_threshold.py --validationImages /content/drive/MyDrive/Wider_Face/yolov5/data/wider_face_samples/images --groundTruth_TxtPath /content/drive/MyDrive/Wider_Face/yolov5/data/wider_face_samples/ground_truths --predicted_TxtPath /content/drive/MyDrive/Wider_Face/yolov5/runs/detect/exp6/labels --npy_path /content/drive/MyDrive/Wider_Face/yolov5/runs
...
def main(opt):
  """
    ...
    Arguments:
      ...
      npy_path            : Directory path to save numpy file result.

  """
  opt.validationImages = str(Path(opt.validationImages).absolute())
  opt.groundTruth_TxtPath = str(Path(opt.groundTruth_TxtPath).absolute())
  opt.predicted_TxtPath = str(Path(opt.predicted_TxtPath).absolute())
  ...
  for Img in batchImages:
    ...
    kl_div_values.append(abs(kl_divergence(f_grd,f_prd)))

  print("density threshold value: ",np.array(kl_div_values).mean())

  if not os.path.exists(opt.npy_path):
    os.makedirs(opt.npy_path)
  np.save(os.path.join(opt.npy_path , "thr.npy"),np.array(kl_div_values).mean())
...
def parse_opt():
    ...
    parser.add_argument('--npy_path', type=str, help='threshold_saved_path')
