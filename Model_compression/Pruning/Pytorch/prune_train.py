...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    def prune_and_reinit(model,percentage):
        ...
        if opt.random_reinit:
            random_model = Model(cfg, ch=3, nc=nc, anchors=hyp.get('anchors')).to(device)
            #random initialization
            for layer_name , param in random_model.named_parameters():
                if 'weight' in layer_name:
                    tensor = param.data.cpu().numpy()              
                    pruned_model_sd_tensor = pruned_model_sd[layer_name].cpu().numpy()                    
                    pruned_random_tensor = np.where(pruned_model_sd_tensor == 0, 0, tensor)
                    param.data = torch.from_numpy(pruned_random_tensor).to(device)
        ...
        if opt.theta0_reinit:
            ...
            theta0_model = Model(cfg = cfg , ch=3, nc=nc, anchors=hyp.get('anchors')).to(device) 
            ckpt = torch.load(opt.theta0_weights, map_location=device)  # load checkpoint
            csd = ckpt['model'].float().state_dict()  # checkpoint state_dict as FP32 :2,93,64,603 bytes
            csd = intersect_dicts(csd, theta0_model.state_dict(), exclude=exclude)  # intersect
            theta0_model.load_state_dict(csd, strict=False)  
            
            #set unpruned weights in model to corresponding theta0_weights
            for layer_name , param in theta0_model.named_parameters():
                if 'weight' in layer_name:
                    tensor = param.data.cpu().numpy()
                    pruned_model_sd_tensor = pruned_model_sd[layer_name].cpu().numpy()
                    pruned_theta_tensor = np.where(pruned_model_sd_tensor == 0, 0, tensor)
                    param.data = torch.from_numpy(pruned_theta_tensor).to(device)
            theta0_model_sd = theta0_model.state_dict()
            model.load_state_dict(theta0_model_sd, strict=False)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            #explicitly set pruned weight gradients to 0
            for name, m in model.named_modules():
                if isinstance(m, nn.Conv2d) and name[:-2] != 'model.24.m':
                    tensor = m.weight.data.cpu().numpy()
                    grad_tensor = m.weight.grad.data.cpu().numpy()
                    grad_tensor = np.where(tensor == 0, 0, grad_tensor)
                    m.weight.grad.data = torch.from_numpy(grad_tensor).to(device)
...
def parse_opt(known=False):
    ...
    parser.add_argument('--prune_iter', type=int, default=0, help='current number of pruning iteration')
    parser.add_argument('--prune_perc', type=int, default=30, help='initial pruning percentage')
    parser.add_argument('--theta0_weights', type=str, default='yolov5s.pt', help='theta0 weights path')
    parser.add_argument('--random_reinit', action='store_true', help='random reinitialization after pruning')
    parser.add_argument('--theta0_reinit', action='store_true', help='theta0 reinitialization after pruning')
