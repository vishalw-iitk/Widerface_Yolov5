...
from dts.Model_compression.Quantization.Pytorch.QAT.yolov5_repo.utils.autoanchor import check_anchors
from dts.Model_compression.Quantization.Pytorch.QAT.yolov5_repo.utils.datasets import create_dataloader
from dts.Model_compression.Quantization.Pytorch.QAT.yolov5_repo.utils.general import labels_to_class_weights, increment_path, labels_to_image_weights, init_seeds, \
    strip_optimizer, get_latest_run, check_dataset, check_file, check_git_status, check_img_size, \
    check_requirements, print_mutation, set_logging, one_cycle, colorstr
from dts.Model_compression.Quantization.Pytorch.QAT.yolov5_repo.utils.google_utils import attempt_download
from dts.Model_compression.Quantization.Pytorch.QAT.yolov5_repo.utils.loss import ComputeLoss
from dts.Model_compression.Quantization.Pytorch.QAT.yolov5_repo.utils.plots import plot_images, plot_labels, plot_results, plot_evolution
from dts.Model_compression.Quantization.Pytorch.QAT.yolov5_repo.utils.torch_utils import ModelEMA, select_device, intersect_dicts, torch_distributed_zero_first, de_parallel
from dts.Model_compression.Quantization.Pytorch.QAT.yolov5_repo.utils.wandb_logging.wandb_utils import WandbLogger, check_wandb_resume
...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    save_dir, epochs, batch_size, weights, single_cls, evolve, data, cfg, resume, noval, nosave, workers, = \
        opt.save_dir, opt.QAT_epochs, opt.batch_size_QAT, opt.weights, opt.single_cls, opt.evolve, opt.data, opt.cfg, \
                ...
                if plots and ni < 3:
                    ...
                    if loggers['tb'] and ni == 0:  # TensorBoard
                        with warnings.catch_warnings():
                            ...
                            # loggers['tb'].add_graph(torch.jit.trace(de_parallel(temp_quantized_model), imgs[0:1].to(device = torch.device('cpu')), strict=False), [])
                elif plots and ni == 10 and loggers['wandb']:
                    wandb_logger.log({'Mosaics': [loggers['wandb'].Image(str(x), caption=x.name) for x in
                                                  save_dir.glob('train*.jpg') if x.exists()]})
            # end batch ------------------------------------------------------------------------------------------------

        # print("s5", model.state_dict())

        # Save model
        with torch.no_grad():
            model.eval()
            quantized_model = torch.quantization.convert(deepcopy(model).to(torch.device('cpu')))

            ema.qat_ema.eval()
            quantized_model_ema = torch.quantization.convert(deepcopy(ema.qat_ema).to(torch.device('cpu')))
        
        # Scheduler
        lr = [x['lr'] for x in optimizer.param_groups]  # for loggers
        scheduler.step()

        # print("s6", model.state_dict())
        ...
        if RANK in [-1, 0]:
            ...
            if (not nosave) or (final_epoch and not evolve):  # if save
                ckpt = {'epoch': epoch,
                        ...
                        'model': deepcopy(quantized_model).state_dict(),
                        'ema': deepcopy(quantized_model_ema).state_dict(),
                        'updates': ema.updates,
...
def parse_opt(known=False):
    ...
    '''
    Arguments modifiable from Pipeline.py and quantization.py as well
    '''
    ...
    parser.add_argument('--project', default='../runs_QAT/train', help='save to project/name')
    parser.add_argument('--name', default='exp', help='save to project/name')
    

    '''
    Permanent default from Pipeline.py and quantization.py
    '''
